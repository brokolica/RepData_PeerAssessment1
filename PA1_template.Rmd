...
```{r meanMedianAndHistogram, echo=TRUE, fig.align='center', fig.height=7}
...
```{r averageDailyPattern, echo=TRUE, fig.align='center'}
...
```
**Mean** total number of steps taken per day with `NA`s replaced is **`r data3.mean`**, while the **median** has value of **`r data3.median`**. After inspecting the new histogram made of dataset with `NA`s included we can see slightly different results. The total amount of days is increased by 8 which has an impact also on the median, which is greater than before and equals to mean number of steps taken.
